...
export class BasicBotTurnEvaluator implements BotTurnEvaluator {
	...
	constructor(
		...
		this.scoreFromType = objectMap<BotTurnEffectType, EffectWeightFormula, EffectWeightFromTurn>(opsFromType, (ops, effectType) => compileEffectWeight(ops, effectType));
	}
	...
	public formatScoredOptions(scored: ScoredOption[], bot: Bot, turnStart: TurnStart): string {
		...
		//  ${s.formula}
		return `  ${toShow.map(s => `${columnarNumber(s.score, 7, 1)}: ${formatAction(s.option.action, bot, turnStart)} [${unique(s.option.effects).join(", ")}]`).join("\n  ")}${tail}`;
	}

	private scoreEffect(effect: BotTurnEffectType, hasVisibleBoard: HasVisibleBoard, effects: BotTurnEffectType[]): number {
		const operation = this.scoreFromType[effect];
		return operation(hasVisibleBoard, effects);
	}

	public scoreEffects(effects: BotTurnEffectType[], hasVisibleBoard: HasVisibleBoard): number {
		return sum(effects.map(e => this.scoreEffect(e, hasVisibleBoard, effects)));
	}
